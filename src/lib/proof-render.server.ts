...
/** Channel view — matches image 2: header + body area + "You joined this channel" pill + UNMUTE bar */
...
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- Channel name + subs (center under avatar area) -->
...
/** Chat list — matches image 1: dark chat list with joined channel highlighted near top */
export function buildChatListSvg(joined: ChannelInfo, others: OtherDialog[]): string {
  ...
  // Build list: joined channel first, then others (up to fill)
  const list: Array<{ title: string; last: string; time: string; unread?: number; joined?: boolean }> = [];
  list.push({
    title: joined.title,
    last: "You joined this channel",
    time: nowTime(),
    joined: true,
  });
