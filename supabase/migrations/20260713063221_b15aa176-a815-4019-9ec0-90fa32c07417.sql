  ...
  SELECT jsonb_build_object(
    ...
    'broadcasts', (
      SELECT jsonb_build_object(
        ...
        'dispatched', count(*) FILTER (WHERE status = 'dispatched' AND (dispatched_at >= _since OR completed_at >= _since)),
        'failed', count(*) FILTER (WHERE status = 'failed' AND (dispatched_at >= _since OR completed_at >= _since))
      )
      FROM public.scheduled_broadcast_items sbi
      WHERE EXISTS (SELECT 1 FROM public.scheduled_broadcasts sb WHERE sb.id = sbi.broadcast_id AND sb.user_id = _uid)
    ),
...
GRANT EXECUTE ON FUNCTION public.health_metrics(int) TO authenticated;
